...
grob1 <- a1[,.N,.(PrecinctGroup=Precinct%/%10)][order(-N)][,
	barchart(as.factor(PrecinctGroup)~N,origin=0,cex=1,main=paste0("Precinct Group '(Decade)' for all Whatcom Ballot Status groups as of 10.29.2024 where N=",nrow(a1)))]
grob2 <- a1[BallotStatus == "Accepted",.N,.(PrecinctGroup=Precinct%/%10)][order(-N)][,
	barchart(as.factor(PrecinctGroup)~N,origin=0,col="navy",cex=1,main=paste0("Precinct Group '(Decade)' for all Whatcom Ballot Status = 'Accepted' on 10.29.2024 where N=",a1[BallotStatus == "Accepted",.N]))]
grob3 <- a1[BallotStatus == "Accepted" | BallotStatus == "Received",.N,.(PrecinctGroup=Precinct%/%10)][order(-N)][,
	barchart(as.factor(PrecinctGroup)~N,origin=0,col="black",cex=1,main=paste0("Precinct Group '(Decade)' for all Whatcom Ballot Status = 'Accepted' or 'Received' on 10.29.2024 where N=",a1[BallotStatus == "Accepted" | BallotStatus == "Received",.N]))]
grob4 <- a1[BallotStatus == "Rejected",.N,.(PrecinctGroup=Precinct%/%10)][order(-N)][,
	barchart(as.factor(PrecinctGroup)~N,origin=0,col="red",cex=1,main=paste0("Precinct Group '(Decade)' for all Whatcom Ballot Status = 'Rejected' on 10.29.2024 where N=",a1[BallotStatus == "Rejected",.N]))]
...
# Note Well: I manually edit the fields (Notepad++) to remove spaces and a comma at the end the column names row.
m1 <- merge(fread("F:\\Politics\\7892872772wm\\WM.txt"),fread("F:\\Politics\\Whatcom2024.11.01.csv"),by.x="StateVoterID",by.y="VoterID",all=TRUE)
...
barchart(N ~ as.factor(Age2024),origin=0,horizontal=FALSE,cex.main=3,main="2024.11.01 Whatcom Accepted ballot matchbacks by Age")]
grid(8,8)
...
m3[,barchart(pctVoted ~ as.factor(Age2024),origin=0,horizontal=FALSE,cex.main=3,main="2024.11.01 Whatcom pct Voted by Age of 2024 GE Active Voters")]
grid(8,8)
